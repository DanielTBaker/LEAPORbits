...
with PdfPages('%s/%s_etas.pdf' %(dirname_save,srce)) as pdf:
    for i in range(times.shape[0]):
        ...
        data=np.load('%s/%s' %(dirname_save,fnames[i]))
        eta_est[i],eta_low[i],eta_high[i]=datareader.eta_from_data(data['I'],data['freq'],data['time'],rbin=data['freq'].shape[0],xlim=30,ylim=1,tau_lim=0*u.ms,srce=srce,prof=data['prof'],template=data['template'])
        pdf.savefig()
    plt.figure(figsize=(8,8))
